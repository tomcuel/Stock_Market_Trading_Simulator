...
void shutdown_server(int server_fd)
{
    while (is_running){
        if (key_pressed('q')){
            ...
            log_message("SYSTEM", "Serveur arrêté", LOG_FILE);
            break;
        }
...
void handle_client(int client_socket)
{
    ...
    while (is_running){
        ...
        // spliting the message into two parts by the " " separator
        std::string client_number = message.substr(0, message.find(" "));
        std::string client_message = message.substr(message.find(" ") + 1);
        std::cout << "Client " << client_number << " : " << client_message << std::endl;
        ...
        try{
            int key = std::stoi(client_message);
            if (value_map.find(key) != value_map.end()){
                response = "Réponse pour la clé " + std::to_string(key) + " : " + std::to_string(value_map[key]);
            } else {
                response = "Erreur : Veuillez entrer un nombre entre 0 et 9.";
            }
        } catch (...){
            response = "Erreur : Veuillez entrer un nombre entre 0 et 9.";
        }
        ...
        // log the response
        log_message("Serveur", response, LOG_FILE);
    }
...
int main()
{
    ...
    clear_file(LOG_FILE);
    log_message("SYSTEM", "Serveur démarré", LOG_FILE);
    ...
    std::cout << "Serveur démarré... (Appuyez sur 'q' pour quitter)\n";
    
    // start a thread to listen for 'q' key press
    std::thread exit_listener(shutdown_server, server_fd);

    std::cout << "Serveur en attente de connexion sur le port " << PORT << "...\n";
    ...
    while (is_running){
        ...
        if (client_socket < 0){
            if (is_running){
                ...
                log_message("SYSTEM", "Erreur de connexion", LOG_FILE);
            }
